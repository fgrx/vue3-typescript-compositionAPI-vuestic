...
onMounted(async () => ressources.value = await ressourceService.getUnvalidatedRessources())

const deleteRessourceAction = (ressourceToDelete: IRessource) => {
    ressources.value = ressources.value.filter(ressource => ressource.id != ressourceToDelete.id)
    if (ressourceToDelete.id) ressourceService.deleteRessourceByID(ressourceToDelete.id)
}

const validateRessourceAction = (ressourceToValidate: IRessource) => {
    ressourceToValidate.isValid = true
    if (ressourceToValidate.id) ressourceService.updateRessource(ressourceToValidate)
    store.dispatch("ressourceModule/addRessourceAction", ressourceToValidate)
    ressources.value = ressources.value.filter(ressource => ressource.id != ressourceToValidate.id)
}
